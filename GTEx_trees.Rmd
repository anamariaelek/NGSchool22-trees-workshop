...
In all the models so far, trees were built by selecting for each split in the tree the variable that minimizes error metric(e.g. classification error or regression RSS). This approach is inherently biased towards selecting variables with many splits, or many missing values. To tackle this, a different approach of building conditional trees employs significance test procedure to select variables. At each tree node, significance is calculated for each covariate by permutations, and the one with the lowest p value is selected for split.
...
```{r eval=FALSE}
dt = fread("data/20220907225044.rmse.txt")
...
```{r}
var_imp <- fread("data/20220908101042.varimp.crf.txt")
```
