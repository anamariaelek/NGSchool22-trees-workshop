...
First we will fit different regression trees to predict individual genes expression. Start with one of the following example genes for which there are pre-calculated grd search results files:  
...
Use `tree()` function to fit decision tree model.
...
Alternatively, we can use `caret::train()` function which can fit a variety of different models, including bagging trees (see `names(getModelInfo())`). It's advantages are that it can automatically perform cross-validation and calculate variable importance.  
